...
public class SVG {
     ...
     */
    public Image renderToPicture(int widthInPixels, int heightInPixels) {
        Image picture = new Image();
        Canvas canvas = picture.beginRecording(widthInPixels, heightInPixels);
        Box viewPort = new Box(0f, 0f, (float) widthInPixels, (float) heightInPixels);

        SVGRenderer renderer = new SVGRenderer(canvas, viewPort, this.renderDPI);
     ...
     */
    public Image renderViewToPicture(String viewId, int widthInPixels, int heightInPixels) {
    ...
    public void renderToCanvas(Canvas canvas, RectF viewPort) {
        ...
        SVGRenderer renderer = new SVGRenderer(canvas, svgViewPort, this.renderDPI);
    ...
    protected static class Length implements Cloneable {
        ...
        public float floatValueX(SVGRenderer renderer) {
            switch (unit) {
                case px:
                    return value;
                case em:
        ...
        public float floatValueY(SVGRenderer renderer) {
        ...
        public float floatValue(SVGRenderer renderer) {
            if (unit == Unit.percent) {
                Box viewPortUser = renderer.getCurrentViewPortInUserUnits();
                if (viewPortUser == null)
                    return value;  // Undefined in this situation - so just return value to avoid an NPE
        ...
        public float floatValue(SVGRenderer renderer, float max) {
            if (unit == Unit.percent) {
